...
match validate_dependencies() {
    Ok(deps) => {
        ...
        if let Some(info) = deps.get("ffmpeg") {
            ...
            has_issues = true;
            println!("{}", "ffmpeg was not detected. Some features may not work properly.".yellow());
            ...
            if input.trim().eq_ignore_ascii_case("y") {
                match install_or_update_dependency("ffmpeg") {
                    ...
                    Err(e) => println!("{}: {}. Will try to continue anyway.", "Warning".yellow(), e)
                }
            } else {
                println!("{}", "Continuing without verified ffmpeg. Some features may not work.".yellow());
            }
        }
