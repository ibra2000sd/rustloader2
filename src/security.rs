...
pub fn validate_path_safety(path: &Path) -> Result<(), AppError> {
    ...
    let canonical_home = match home_dir.canonicalize() {
        ...
        Err(_) => {
            return Err(AppError::PathError(
                "Could not canonicalize home directory".to_string(),
            ))
        }
    };
...
pub fn detect_command_injection(input: &str) -> bool {
    ...
    // Check for attempted escaping of quotes - still important
    if input.contains("\\\"") || input.contains("\\'") {
        return true;
    }

    // Check for environment variable access - but allow $ in URLs since
    // it can appear legitimately in some URLs
    if input.contains("${") {
        return true;
    }
